...
const URLS = {
  ...
  'avalanche-fuji':
    'https://avax-fuji.g.alchemy.com/v2/hR5uamq-K43YZYAJldc7lpxZ2MOv0Qbk',
  'arbitrum-sepolia':
    'https://arb-sepolia.g.alchemy.com/v2/hR5uamq-K43YZYAJldc7lpxZ2MOv0Qbk',
  'polygon-amoy':
    'https://polygon-amoy.g.alchemy.com/v2/hR5uamq-K43YZYAJldc7lpxZ2MOv0Qbk',
}
...
app.post('/balance', async (c) => {
  ...
  try {
    ...
    const chainPromises = Object.entries(URLS).map(([chainName, url]) =>
      fetchChainBalance(chainName, url),
    )
    ...
    const formattedResults = results.map((result) => {
      ...
      const chainUsdcAddress =
        USDC_ADDRESSES[
          result.chain as keyof typeof USDC_ADDRESSES
        ]?.toLowerCase()

      // Find the USDC token balance for this chain
      const usdcBalance = tokenBalances.find(
        (token) => token.contractAddress?.toLowerCase() === chainUsdcAddress,
      )
...
app.post('/history', async (c) => {
  try {
    ...
    const fetchBlockData = async (chainName: string, blockNum: string) => {
      try {
        const blockResponse = await fetch(
          URLS[chainName as keyof typeof URLS],
          {
            method: 'POST',
            headers: HEADERS,
            body: JSON.stringify({
    ...
    try {
      ...
      const chainPromises = Object.entries(URLS).map(([chainName, url]) =>
        fetchChainHistory(chainName, url),
      )
      ...
      const formattedResults = results.map(async (result) => {
        ...
        const formattedTransfers = await Promise.all(
          transfers.map(async (transfer: Transfer) => {
            const timestamp = await fetchBlockData(
              result.chain,
              transfer.blockNum,
            )
...
app.get('/ens', async (c) => {
  try {
    ...
    const provider = new ethers.JsonRpcProvider(
      'https://eth-mainnet.g.alchemy.com/v2/hR5uamq-K43YZYAJldc7lpxZ2MOv0Qbk',
    )
    ...
    return c.json(
      { error: 'Failed to lookup ENS name', message: (error as Error).message },
      500,
    )
  }
})
